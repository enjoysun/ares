package cn.gov.zcy.ares.adrasteia.core.chain;

import cn.gov.zcy.ares.adrasteia.core.envluation.AresValueParser;
import cn.gov.zcy.ares.adrasteia.core.envluation.ExpressionEnum;
import cn.gov.zcy.ares.adrasteia.core.parser.ContextParser;
import cn.gov.zcy.ares.adrasteia.meta.LogPersistContext;
import cn.gov.zcy.ares.adrasteia.meta.ParserData;
import org.aspectj.lang.JoinPoint;

/**
 * @author <a href="mailto:youming@cai-inc.com">斜照</a>
 * @datetime 2021-12-13 19:07:08
 */
public class ContextFilter implements LogRecordPrepareFilter {
    public ContextFilter(AresValueParser logValueParser, String contextExpressionText) {
        this.logValueParser = logValueParser;
        this.contextExpressionText = contextExpressionText;
    }

    private AresValueParser logValueParser;

    private String contextExpressionText;

    @Override
    public void doFilter(LogPersistContext persistContext, JoinPoint joinPoint, Object result, FilterChain chain) {
        ParserData<String> bizData = ParserData.<String>builder().expressionText(contextExpressionText).expressionEnum(ExpressionEnum.CONTEXT).joinPoint(joinPoint).clazz(String.class).joinPoint(joinPoint).result(result).build();
        new ContextParser().invoke(logValueParser, bizData, persistContext);
        chain.doFilter(persistContext, joinPoint, result, chain);
    }
}
